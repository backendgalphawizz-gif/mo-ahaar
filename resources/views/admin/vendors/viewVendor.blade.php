...
<div class="page-body">
    <div class="container-fluid">
        <div class="d-flex flex-wrap align-items-center gap-3 mb-4">
            <a href="{{ route('admin.vendors') }}" class="btn btn-outline-secondary btn-sm"><i class="ri-arrow-left-line"></i></a>
            <div>
                <h5 class="mb-0">Vendor Details</h5>
                <div class="d-flex align-items-center gap-2 mt-1">
                    <span class="fw-semibold">{{ $vendor->vendor_code }}</span>
                    <span class="badge {{ $statusClass }}">{{ strtoupper($statusKey) }}</span>
                </div>
            </div>
            <div class="ms-auto d-flex gap-2">
                @if($statusKey !== 'rejected')
                    <form method="POST" action="{{ route('admin.vendors.approval-status', $vendor->vendor_id) }}">
                        ...
                        <button class="btn btn-outline-danger btn-sm"><i class="ri-close-line me-1"></i>Reject</button>
                    </form>
                @endif
                @if($statusKey !== 'approved')
                    <form method="POST" action="{{ route('admin.vendors.approval-status', $vendor->vendor_id) }}">
                        ...
                        <button class="btn btn-success btn-sm"><i class="ri-check-line me-1"></i>Approve</button>
                    </form>
                @endif
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-3">
                <div class="card dashboard-card h-100"><div class="card-body">
                    ...
                    <h4 class="mb-0">₹{{ number_format((float) ($vendor->wallet_balance ?? 0), 0) }}</h4>
                </div></div>
            </div>
            <div class="col-md-3">
                <div class="card dashboard-card h-100"><div class="card-body">
                    <small class="text-muted">Withdrawal Amount</small>
                    <h4 class="mb-0">₹{{ number_format((float) ($vendor->withdrawal_amount ?? 0), 0) }}</h4>
                </div></div>
            </div>
            <div class="col-md-3">
                <div class="card dashboard-card h-100"><div class="card-body d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h4 class="mb-0">{{ number_format((float) ($vendor->commission_percent ?? 0), 0) }}%</h4>
                    </div>
                    <button class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#commissionModal"><i class="ri-pencil-line"></i></button>
                </div></div>
            </div>
            <div class="col-md-3">
                <div class="card dashboard-card h-100"><div class="card-body">
                    <small class="text-muted">Refund Balance</small>
                    <h4 class="mb-0">₹{{ number_format((float) ($vendor->refund_balance ?? 0), 0) }}</h4>
                </div></div>
            </div>
        </div>
                ...
                @if(($tab ?? 'details') === 'details')
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="row g-2">
                                @foreach(['aadhaar_card' => 'Aadhaar', 'pan_card' => 'PAN', 'business_logo' => 'Logo', 'shop_image' => 'Shop', 'business_banner' => 'Banner'] as $field => $label)
                                    <div class="col-md-4">
                                        <div class="doc-box p-3 text-center border rounded">
                                            ...
                                            @if(!empty($vendor->{$field}))
                                                ...
                                                @endphp
                                                <a href="{{ asset($docPath) }}" target="_blank">View File</a>
                                            @else
...
.doc-box { min-height: 90px; background: #f8f8f8; }
</style>
@endsection
