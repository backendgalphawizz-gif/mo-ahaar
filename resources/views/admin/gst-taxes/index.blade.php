...
<div class="page-body">
    <div class="container-fluid">
        <div class="title-header option-title d-flex align-items-center flex-wrap gap-2 mb-4">
            ...
            <a href="{{ route('admin.gst-taxes.create') }}" class="btn btn-theme btn-sm ms-auto">
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-body py-2">
                <form method="GET" action="{{ route('admin.gst-taxes.index') }}" class="d-flex gap-2">
                    ...
                    @if($search)
                    <a href="{{ route('admin.gst-taxes.index') }}" class="btn btn-sm btn-outline-danger">Clear</a>
                    @endif
                </form>
            </div>
        </div>
                            ...
                            <tr id="gst-row-{{ $tax->id }}">
                                ...
                                <td>
                                    <ul class="d-flex gap-2 mb-0 list-unstyled">
                                        <li>
                                            <a href="{{ route('admin.gst-taxes.edit', $tax->id) }}" title="Edit">
                                                <i class="ri-pencil-line"></i>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="javascript:void(0)" class="delete-gst-btn"
                                                data-form-id="delete-gst-form-{{ $tax->id }}"
                                                data-name="{{ $tax->name }}" title="Delete">
                                                <i class="ri-delete-bin-line text-danger"></i>
                                            </a>
                                            <form id="delete-gst-form-{{ $tax->id }}" method="POST"
                                                action="{{ route('admin.gst-taxes.destroy', $tax->id) }}"
                                                class="d-none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
