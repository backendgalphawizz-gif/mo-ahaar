...
<div class="page-body">
    <div class="container-fluid">
        <div class="title-header option-title d-flex align-items-center mb-4">
            <h5><i class="ri-bank-card-line me-2"></i>{{ $title }}</h5>
        </div>
        ...
        <form method="POST" action="{{ route('admin.settings.payment-methods.update') }}" id="paymentMethodsForm" novalidate>
            @csrf

            {{-- ─── RAZORPAY ─────────────────────────────────────────────────── --}}
            @php $rz = $gateways->get('razorpay'); $rzSettings = $rz->settings ?? []; @endphp
                ...
                <div class="card-header bg-white border-bottom d-flex align-items-center justify-content-between py-3 px-4">
                    ...
                    <div class="form-check form-switch mb-0 d-flex align-items-center gap-2">
                        ...
                        <input class="form-check-input gateway-toggle m-0" type="checkbox"
                               ...
                               value="1"
                               {{ $rz && $rz->is_enabled ? 'checked' : '' }}>
                                                                                           
                        <label class="form-check-label mb-0" for="razorpay_enabled">
                            {{ $rz && $rz->is_enabled ? 'Enabled' : 'Disabled' }}
                        </label>
                    </div>
                </div>
                <div class="card-body gateway-fields {{ $rz && $rz->is_enabled ? '' : 'opacity-50' }}" id="razorpay_fields">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Razorpay Key ID <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('gateways.razorpay.settings.key_id') is-invalid @enderror"
                                   name="gateways[razorpay][settings][key_id]"
                                   value="{{ old('gateways.razorpay.settings.key_id', $rzSettings['key_id'] ?? '') }}"
                                   placeholder="rzp_live_xxxxxxxxxxxxxxxxxx">
                            <small class="text-muted">Your Razorpay Key ID from the dashboard.</small>
                            @error('gateways.razorpay.settings.key_id')
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="password" class="form-control @error('gateways.razorpay.settings.secret_key') is-invalid @enderror"
                                       id="razorpay_secret_key"
                                       name="gateways[razorpay][settings][secret_key]"
                                       value="{{ old('gateways.razorpay.settings.secret_key', $rzSettings['secret_key'] ?? '') }}"
                            ...
                            @error('gateways.razorpay.settings.secret_key')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="password" class="form-control @error('gateways.razorpay.settings.webhook_secret') is-invalid @enderror"
                                       id="razorpay_webhook_secret"
                                       name="gateways[razorpay][settings][webhook_secret]"
                                       value="{{ old('gateways.razorpay.settings.webhook_secret', $rzSettings['webhook_secret'] ?? '') }}"
