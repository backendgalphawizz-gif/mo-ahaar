        ...
        <div class="container-fluid">
            <div class="title-header option-title d-flex align-items-center flex-wrap gap-2 mb-4">
                ...
                <a class="btn btn-outline-secondary btn-sm ms-auto"
                    href="{{ route('admin.products', array_filter(['segment' => $segment ?? null])) }}">
            ...
            <form method="POST" action="{{ route('admin.store-product') }}" enctype="multipart/form-data" id="productForm">
                @csrf
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="product_name"
                                            ...
                                            placeholder="Product name" value="{{ old('product_name') }}" maxlength="100"
                                            data-product-name required>
                                        @error('product_name')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <select name="target_user_type" id="target_user_type"
                                            class="form-select @error('target_user_type') is-invalid @enderror" required>
                                            <option value="">Select retailer or wholesaler</option>
                                            <option value="{{ \App\Models\Product::TARGET_RETAILER }}" {{ (string) old('target_user_type', $defaultTarget ?? '') === \App\Models\Product::TARGET_RETAILER ? 'selected' : '' }}>Retailer
                                            </option>
                                            <option value="{{ \App\Models\Product::TARGET_WHOLESALER }}" {{ (string) old('target_user_type', $defaultTarget ?? '') === \App\Models\Product::TARGET_WHOLESALER ? 'selected' : '' }}>Wholesaler
                                            </option>
                                        </select>
                                        <small class="text-muted">Retailer and wholesaler catalogs are separate in the list
                                            and app.</small>
                                        @error('target_user_type')<p class="invalid-feedback d-block mb-0">{{ $message }}
                                        </p>@enderror
                                    </div>
                                    <div class="col-md-6">
                                        <div> &nbsp;</div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select @error('category_id') is-invalid @enderror"
                                            ...
                                            @foreach($categoryList as $category)
                                                <option value="{{ $category->category_id }}" {{ (string) old('category_id') === (string) $category->category_id ? 'selected' : '' }}>
                                                    {{ $category->category_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('category_id')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        @error('sub_category_id')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-12">
                                        ...
                                        <input type="url" name="video"
                                            ...
                                            placeholder="https://www.youtube.com/watch?v=..." value="{{ old('video') }}">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between mt-1">
                                            ...
                                            <small id="desc_counter" class="text-muted"><span
                                                    id="desc_count">{{ strlen(old('product_description', '')) }}</span> /
                                                220</small>
                                        </div>
                                        @error('product_description')<p class="invalid-feedback d-block mb-0">{{ $message }}
                                        </p>@enderror
                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" step="0.01" min="0" name="mrp_price" id="mrp_price"
                                            class="form-control @error('mrp_price') is-invalid @enderror" placeholder="0.00"
                                            value="{{ old('mrp_price') }}" required>
                                        @error('mrp_price')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        @error('discount')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="stock_status"
                                            ...
                                            <option value="backorder" {{ old('stock_status') === 'backorder' ? 'selected' : '' }}>On backorder</option>
                                        </select>
                                        @error('stock_status')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-md-4 {{ (string) old('target_user_type', $defaultTarget ?? '') === \App\Models\Product::TARGET_WHOLESALER ? '' : 'd-none' }}"
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" name="sku" maxlength="100"
                                            ...
                                            placeholder="Leave blank to auto-generate" value="{{ old('sku') }}">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="gst_calculation_type"
                                            class="form-select @error('gst_calculation_type') is-invalid @enderror">
                                            <option value="excluded" {{ old('gst_calculation_type', \App\Models\Product::GST_EXCLUDED) === \App\Models\Product::GST_EXCLUDED ? 'selected' : '' }}>Excluded GST</option>
                                            <option value="included" {{ old('gst_calculation_type') === \App\Models\Product::GST_INCLUDED ? 'selected' : '' }}>Included GST</option>
                                        </select>
                                        <small class="text-muted">Choose if selling price is GST-exclusive or
                                            GST-inclusive.</small>
                                        @error('gst_calculation_type')<p class="invalid-feedback d-block mb-0">
                                        {{ $message }}</p>@enderror
                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="gst_tax_id"
                                            class="form-select @error('gst_tax_id') is-invalid @enderror">
                                            <option value="">No GST / Exempt</option>
                                            @foreach($gstTaxes as $gst)
                                                <option value="{{ $gst->id }}" {{ (string) old('gst_tax_id') === (string) $gst->id ? 'selected' : '' }}>
                                        ...
                                        <small class="text-muted">GST is added on top of the selling price. Leave blank if
                                            exempt.</small>
                                        @error('gst_tax_id')<p class="invalid-feedback d-block mb-0">{{ $message }}</p>
                                        @enderror
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="file" name="product_image" id="product_image"
                                        class="d-none @error('product_image') is-invalid @enderror" accept="image/*">
                                    <label for="product_image" id="thumbDropZone"
                                        class="img-dropzone @error('product_image') has-error @enderror">
                                    ...
                                    @error('product_image')<p class="text-danger small mt-2 mb-0"><i
                                    class="ri-error-warning-line me-1"></i>{{ $message }}</p>@enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="file" name="gallery_images[]" id="gallery_images"
                                        class="d-none @error('gallery_images') is-invalid @enderror @error('gallery_images.*') is-invalid @enderror"
                                        accept="image/*" multiple>
                                    <label for="gallery_images" id="galleryDropZone"
                                        class="img-dropzone img-dropzone--sm @error('gallery_images') has-error @enderror @error('gallery_images.*') has-error @enderror">
                                    ...
                                    @error('gallery_images')<p class="text-danger small mt-2 mb-0"><i
                                    class="ri-error-warning-line me-1"></i>{{ $message }}</p>@enderror
                                    @error('gallery_images.*')<p class="text-danger small mt-2 mb-0"><i
                                    class="ri-error-warning-line me-1"></i>{{ $message }}</p>@enderror
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var initialSubCategoryId = {!! json_encode(old('sub_category_id', '')) !!};
